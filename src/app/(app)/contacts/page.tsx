    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        ...
        <DialogFooter>
          ...
          <Button className="bg-[#AA8038] text-white hover:bg-[#D78C00]" onClick={() => void save()} disabled={saving}>{saving ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}{contact ? "Save Changes" : "Create Contact"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-[#f8f9fc]">
      ...
      <Tabs defaultValue="overview" className="flex min-h-0 flex-1 flex-col">
        <TabsList className="w-full justify-start rounded-none border-b bg-white px-4 py-0"><TabsTrigger value="overview" className="h-10 rounded-none border-b-2 border-transparent px-4 data-[state=active]:border-[#AA8038] data-[state=active]:bg-transparent data-[state=active]:text-[#AA8038]">Overview</TabsTrigger><TabsTrigger value="organization" className="h-10 rounded-none border-b-2 border-transparent px-4 data-[state=active]:border-[#AA8038] data-[state=active]:bg-transparent data-[state=active]:text-[#AA8038]">Organization</TabsTrigger><TabsTrigger value="insights" className="h-10 rounded-none border-b-2 border-transparent px-4 data-[state=active]:border-[#AA8038] data-[state=active]:bg-transparent data-[state=active]:text-[#AA8038]">AI Insights</TabsTrigger></TabsList>
        <TabsContent value="overview" className="mt-0 flex-1 overflow-y-auto p-5 bg-gray-50">
          <div className="rounded-xl border bg-white p-4">
            <div className="grid gap-2 text-sm sm:grid-cols-2">
              <div>{hasText(contact.email) ? <a className="text-[#B67600] hover:underline" href={`mailto:${contact.email ?? ""}`}>{contact.email}</a> : <span className="text-gray-400">No email</span>}</div>
